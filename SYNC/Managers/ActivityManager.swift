...
class ActivityManager: ObservableObject {
    ...
    func loadMoreActivities(currentUserId: String, matchedUserIds: [String], limit: Int = 10) async throws -> [Activity] {
        ...
        // ✅ ADDED: Get current time for filtering
        ...
        for batch in batchedIds {
            let query = activitiesCollection()
                .whereField("creatorId", in: batch)
                .whereField("startTime", isGreaterThan: now) // ✅ ADDED: Only future activities
                .order(by: "startTime", descending: true) // ✅ CHANGED: Order by startTime ascending
                .start(afterDocument: lastDoc)
                .limit(to: limit)
        ...
        // ✅ CHANGED: Sort by startTime ascending (soonest first)
        allActivities.sort { $0.startTime < $1.startTime }
        let limitedActivities = Array(allActivities.prefix(limit))
    ...
    func listenForNewActivities(currentUserId: String, matchedUserIds: [String], onNewActivities: @escaping ([Activity]) -> Void) {
        ...
        activitiesListener = activitiesCollection()
            .whereField("creatorId", in: limitedMatches)
            .whereField("startTime", isGreaterThan: now) // ✅ ADDED: Only future activities
            .order(by: "startTime", descending: true) // ✅ CHANGED: Order by startTime
            .addSnapshotListener { [weak self] snapshot, error in
...
extension ActivityManager {
    ...
    /// Fetches the initial batch of activities with optional date range filtering
    /// ✅ UPDATED: Now only fetches activities with startTime > current date
    /// - Parameters:
    ///   - currentUserId: The current user's ID
    ///   - matchedUserIds: Array of user IDs that the current user has matched with
    ///   - dateRange: Optional date range filter to apply server-side
    ///   - limit: Number of activities to fetch (default: 20)
    /// - Returns: Array of activities sorted by startTime ascending (soonest first)
    func fetchInitialActivities(
        currentUserId: String,
        matchedUserIds: [String],
        dateRange: DateRangeFilter = .all,
        ...
        // ✅ ADDED: Get current time for filtering out past activities
        let now = Timestamp(date: Date())
        
        // Batch the matched user IDs properly
        let batchedIds = matchedUserIds.chunked(into: 10)
        var allActivities: [Activity] = []
        ...
        for batch in batchedIds {
            ...
            // ✅ UPDATED: Apply date range filter with future-only constraint
            if let range = dateRange.dateRange {
                // Ensure we only get activities in the future within the specified range
                let effectiveStart = max(range.start, Date())
                
                query = query
                    .whereField("startTime", isGreaterThanOrEqualTo: Timestamp(date: effectiveStart))
                    .whereField("startTime", isLessThan: Timestamp(date: range.end))
                    .order(by: "startTime", descending: true) // ✅ Soonest first
            } else {
                // ✅ ADDED: Default filter - only future activities
                query = query
        ...
        // ✅ CHANGED: Always sort by startTime ascending (soonest activities first)
        allActivities.sort { $0.startTime < $1.startTime }
        ...
        print("🔥 Fetched \(limitedActivities.count) future activities with date filter: \(dateRange.displayName)")
    ...
    func loadMoreActivities(
        ...
        // ✅ CHANGED: Always sort by startTime ascending
        allActivities.sort { $0.startTime < $1.startTime }
        ...
        print("🔥 Loaded \(limitedActivities.count) more future activities")
