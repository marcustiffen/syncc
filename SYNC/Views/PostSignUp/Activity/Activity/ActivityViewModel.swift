...
class ActivityViewModel: ObservableObject {
    ...
    // ✅ NEW: Separate search text that doesn't trigger filtering
    ...
    private func setupFilterObserver() {
        // ✅ CHANGED: Only observe filter changes, NOT searchText
    ...
    // ✅ OPTIMIZED: Filter logic remains the same but now only runs on explicit action
    ...
    func clearFilters(currentUserId: String) async {
        filter = ActivityFilter(userLocation: filter.userLocation)
        searchText = "" // ✅ Also clear the search text field
        await refreshActivities(currentUserId: currentUserId)
    }
