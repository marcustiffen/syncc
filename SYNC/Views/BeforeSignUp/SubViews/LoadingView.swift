...
struct LoadingView: View {
    ...
    var body: some View {
        ZStack {
            FullSine()
                .trim(from: 0, to: fullSineProgress)
                .stroke(.syncBlack, style: StrokeStyle(lineWidth: 16, lineCap: .round))
...
struct FullSine: Shape {
    func path(in rect: CGRect) -> Path {
        Path { path in
            let amplitude = rect.height / 2 // Increased amplitude for taller waves
            let wavelength = rect.width
...
struct JoiningSine: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        Path { path in
            //Top
            let amplitude = rect.height / 2 // Increased amplitude to match FullSine
            let wavelength = rect.width
